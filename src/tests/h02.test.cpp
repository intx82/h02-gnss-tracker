...
TEST(LocationTTest, SerializeDeserialize) {
    ...
    location_t original("1234567890", 22.3942, 114.0088, 0.00, 348, date_time, 0xffffffff);
    std::string serialized = original.serialize();

    location_t deserialized(serialized);
    EXPECT_EQ(original.device_id, deserialized.device_id);
    EXPECT_NEAR(original.latitude, deserialized.latitude, 0.0001);
    EXPECT_NEAR(original.longitude, deserialized.longitude, 0.0001);
